...
- Keep the two implementations in lock-step. A change to the questions (`ts/src/judge.ts`,
  `csharp/Frugal/Judge.cs`) or the policy (`ts/src/policy.ts`, `csharp/Frugal/Policy.cs`) must
  land in both, with the matching test in `ts/test/` and `csharp/Frugal.Tests/`.
...
C# (`cd csharp`): `dotnet test`, `dotnet run --project Frugal -- route "<prompt>"`, `dotnet run --project Frugal -- serve`
